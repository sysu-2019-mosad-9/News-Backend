...
ssh $SERVER_USERNAME@$SERVER_IP -o StrictHostKeyChecking=no -i /tmp/deploy_rsa '/bin/bash -c "cd /root/newsback && ls && git pull origin master && ROCKET_ENV=production cargo build --release && ./target/release/news-backend"'